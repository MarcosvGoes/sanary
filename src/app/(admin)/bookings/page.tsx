...
type VisibleBookingStatus = Exclude<BookingStatus, "BLOCKED">

const bookingStatusConfig: Record<
  VisibleBookingStatus,
  {
    label: string
    dot: string
    border: string
      ...
      {bookings.map((booking) => {
        const statusConfig =
          booking.status !== "BLOCKED"
            ? bookingStatusConfig[booking.status as VisibleBookingStatus]
            : null

        const cardBorder =
          statusConfig?.border ?? "border-border"
        ...
        return (
          <div
            key={booking.id}
            className={`border ${cardBorder} max-w-[500px] md:mx-auto rounded-xl p-5 bg-white shadow-sm space-y-4`}
          >
            <div>
              ...
              {booking.status !== "BLOCKED" && (() => {
                const config = bookingStatusConfig[booking.status as VisibleBookingStatus]

                return (
                  <div className={`flex items-center gap-x-2 font-medium ${config.text}`}>
                    <span
                      className={`
        ...
        `}
            ...
            <div>
              {booking.status !== "CANCELED" && booking.status !== "BLOCKED" ? (
                <Dialog>
